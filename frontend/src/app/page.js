...
function renderMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    if (paraLines.length > 0) {
      elements.push(<p key={key++} className="md-p">{inlineMarkdown(paraLines.join(" "))}</p>)
    } else {
      i++
    }
  }
...
function inlineMarkdown(text) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[0].startsWith("**")) parts.push(<strong key={k++}>{match[2]}</strong>)
    else if (match[0].startsWith("*")) parts.push(<em key={k++}>{match[3]}</em>)
    else if (match[0].startsWith("`")) parts.push(<code key={k++} className="md-inline-code">{match[4]}</code>)
...
const dataSources = [
  ...
  {
    id: "csv", name: "CSV", label: "File Upload", accept: ".csv", placeholder: null, color: "#10b981", bg: "#f0fdf4",
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="8" y1="13" x2="16" y2="13" /><line x1="8" y1="17" x2="16" y2="17" /><line x1="8" y1="9" x2="10" y2="9" /></svg>,
  },
  {
    id: "gsheet", name: "Google Sheets", label: "URL", accept: null, placeholder: "https://docs.google.com/spreadsheets/d/...", color: "#f59e0b", bg: "#fffbeb",
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="3" y1="15" x2="21" y2="15" /><line x1="9" y1="3" x2="9" y2="21" /></svg>,
  },
  {
    id: "github", name: "GitHub CSV", label: "Raw URL", accept: null, placeholder: "https://raw.githubusercontent.com/...", color: "#8b5cf6", bg: "#f5f3ff",
  ...
  {
    id: "mysql", name: "MySQL", label: "Connection String", accept: null, placeholder: "mysql://user:password@host:3306/db", color: "#ef4444", bg: "#fef2f2",
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M21 5v4c0 1.66-4.03 3-9 3S3 10.66 3 9V5" /><path d="M21 9v4c0 1.66-4.03 3-9 3S3 14.66 3 13V9" /><path d="M21 13v4c0 1.66-4.03 3-9 3s-9-1.34-9-3v-4" /></svg>,
  },
  {
    id: "postgres", name: "PostgreSQL", label: "Connection String", accept: null, placeholder: "postgresql://user:password@host:5432/db", color: "#3b82f6", bg: "#eff6ff",
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a10 10 0 100 20A10 10 0 0012 2z" /><path d="M12 8v8M8 12h8" /></svg>,
  },
]
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .logo-name { font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 700; color: #f8fafc; letter-spacing: -0.3px; }
        .logo-badge { font-size: 10px; font-weight: 500; background: rgba(99,102,241,0.15); color: #818cf8; border: 1px solid rgba(99,102,241,0.3); padding: 2px 8px; border-radius: 100px; letter-spacing: 0.5px; text-transform: uppercase; }
        ...
        .source-btn.active { background: rgba(99,102,241,0.1); border-color: rgba(99,102,241,0.4); box-shadow: 0 0 20px rgba(99,102,241,0.15), inset 0 0 0 1px rgba(99,102,241,0.2); }
        .source-icon-wrap { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
        ...
        .drop-zone:hover { border-color: rgba(99,102,241,0.5); background: rgba(99,102,241,0.07); }
        .drop-icon { width: 36px; height: 36px; margin: 0 auto 10px; color: #6366f1; opacity: 0.7; }
        .drop-title { font-size: 13px; font-weight: 500; color: #94a3b8; margin-bottom: 4px; }
        .drop-sub { font-size: 11px; color: #475569; }
        .file-chosen { font-size: 12px; color: #818cf8; font-weight: 500; }
        .text-input { width: 100%; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 12px 14px; color: #e2e8f0; font-family: 'DM Mono', monospace; font-size: 12px; outline: none; transition: all 0.2s; margin-bottom: 12px; }
        .text-input::placeholder { color: #334155; }
        .text-input:focus { border-color: rgba(99,102,241,0.5); background: rgba(99,102,241,0.05); box-shadow: 0 0 0 3px rgba(99,102,241,0.1); }
        .connect-btn { width: 100%; padding: 12px; border-radius: 12px; border: none; cursor: pointer; font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 600; letter-spacing: 0.3px; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.2s; }
        .connect-btn:not(:disabled) { background: linear-gradient(135deg, #6366f1, #818cf8); color: white; box-shadow: 0 4px 20px rgba(99,102,241,0.3); }
        .connect-btn:not(:disabled):hover { transform: translateY(-1px); box-shadow: 0 8px 28px rgba(99,102,241,0.4); }
        ...
        .query-textarea { flex: 1; background: transparent; border: none; outline: none; color: #e2e8f0; font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 300; line-height: 1.6; resize: none; min-height: 60px; max-height: 180px; }
        .query-textarea::placeholder { color: #334155; }
        .send-btn { width: 42px; height: 42px; border-radius: 12px; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.2s; }
        .send-btn:not(:disabled) { background: linear-gradient(135deg, #6366f1, #818cf8); box-shadow: 0 4px 16px rgba(99,102,241,0.35); }
        .send-btn:not(:disabled):hover { transform: scale(1.06); box-shadow: 0 6px 22px rgba(99,102,241,0.5); }
        ...
        .answer-label-dot { width: 6px; height: 6px; border-radius: 50%; background: #10b981; box-shadow: 0 0 6px #10b981; }
        ...
        .md-h3 { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 600; color: #a5b4fc; text-transform: uppercase; letter-spacing: 0.6px; margin: 16px 0 6px; }
        .md-p { color: #94a3b8; margin: 0 0 10px; line-height: 1.75; }
        .md-p strong { color: #e2e8f0; font-weight: 600; }
        .md-ul { list-style: none; padding: 0; margin: 0 0 12px; display: flex; flex-direction: column; gap: 6px; }
        .md-ul li { position: relative; padding-left: 18px; color: #94a3b8; font-size: 14px; line-height: 1.65; }
        .md-ul li::before { content: ''; position: absolute; left: 2px; top: 9px; width: 6px; height: 6px; border-radius: 50%; background: #6366f1; opacity: 0.7; }
        .md-ul li strong { color: #c7d2fe; font-weight: 600; }
        .md-ol { list-style: none; padding: 0; margin: 0 0 12px; counter-reset: ol-counter; display: flex; flex-direction: column; gap: 10px; }
        .md-ol li { counter-increment: ol-counter; position: relative; padding: 10px 14px 10px 44px; background: rgba(99,102,241,0.05); border: 1px solid rgba(99,102,241,0.12); border-radius: 10px; color: #94a3b8; font-size: 14px; line-height: 1.6; }
        .md-ol li::before { content: counter(ol-counter); position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 22px; height: 22px; background: rgba(99,102,241,0.2); color: #a5b4fc; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-family: 'Syne', sans-serif; font-size: 11px; font-weight: 700; line-height: 22px; text-align: center; }
        .md-ol li strong { color: #c7d2fe; font-weight: 600; }
        .md-subitem { padding-left: 28px !important; background: transparent !important; border: none !important; border-left: 2px solid rgba(99,102,241,0.15) !important; border-radius: 0 !important; margin-left: 12px; }
        .md-subitem::before { display: none !important; }
        .md-hr { border: none; border-top: 1px solid rgba(255,255,255,0.06); margin: 16px 0; }
        .md-code-block { background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.06); border-radius: 10px; overflow: hidden; margin: 10px 0; }
        .md-code-lang { display: block; padding: 6px 14px; font-size: 10px; font-family: 'DM Mono', monospace; color: #6366f1; background: rgba(99,102,241,0.08); border-bottom: 1px solid rgba(99,102,241,0.1); text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        .thinking-dots span { width: 6px; height: 6px; border-radius: 50%; background: #6366f1; animation: blink 1.2s ease-in-out infinite; }
      ...
      <div className="app-shell">
        ...
        <div className="content">
          ...
          <div className="hero">
            ...
            <h1 className="hero-title">Query any data source<br /><span>with plain English</span></h1>
            <p className="hero-sub">Connect your database, ask questions naturally — get instant, intelligent answers powered by AI.</p>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div className="panel">
                ...
                <div className="source-grid">
                  {dataSources.map(src => (
                    <button key={src.id} className={`source-btn${activeTab === src.id ? " active" : ""}`}
                      onClick={() => { setActiveTab(src.id); setFile(null); setUrlValue(""); setConnStr("") }}>
                      {activeTab === src.id && <span className="active-pip" />}
                      <div className="source-icon-wrap" style={{ background: activeTab === src.id ? `${src.color}18` : "rgba(255,255,255,0.04)", color: activeTab === src.id ? src.color : "#475569" }}>
                        {src.icon}
                      </div>
                ...
                <div className="connect-area">
                  ...
                  {isFileSource ? (
                    <>
                      <input type="file" accept={activeSource.accept} style={{ display: "none" }} ref={fileRef} onChange={e => setFile(e.target.files?.[0] || null)} />
                      <div className="drop-zone" onClick={() => fileRef.current?.click()}>
                        ...
                        {file ? <p className="file-chosen">{file.name}</p> : <><p className="drop-title">Drop file or click to browse</p><p className="drop-sub">{activeSource.accept} files only</p></>}
                      </div>
                    </>
                  ) : (
                    <input type="text" className="text-input" placeholder={activeSource.placeholder}
                      value={isUrlSource ? urlValue : connStr}
                      onChange={e => isUrlSource ? setUrlValue(e.target.value) : setConnStr(e.target.value)} />
                  )}
                  <button className="connect-btn" onClick={uploadDB} disabled={isUploading || !canConnect}>
                    {isUploading ? <><div className="spinner" /> Connecting…</> : <>
                ...
                <div className="history-box">
                  <div className="panel-header">
                    <div className="panel-icon" style={{ background: "rgba(99,102,241,0.08)" }}>
                      <svg viewBox="0 0 24 24" fill="none" stroke="#818cf8" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                    </div>
                  ...
                  {history.map((h, i) => (
                    <div key={i} className="history-item" onClick={() => { setAnswer(h.a); setAnimateAnswer(false) }}>
                      ...
                      <p className="history-a">{stripMd(h.a).slice(0, 90)}{stripMd(h.a).length > 90 ? "…" : ""}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="query-box">
                <div className="query-input-wrap">
                  <textarea className="query-textarea"
                    ...
                    onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (dbName && question.trim() && !isQuerying) askQuestion() } }}
                    disabled={!dbName} rows={2} />
                  <button className="send-btn" onClick={askQuestion} disabled={!dbName || isQuerying || !question.trim()}>
                    {isQuerying ? <div className="spinner" /> : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></svg>}
                  </button>
                </div>
                <div className="query-hints">
                  {["Top 10 by revenue", "Count by category", "Monthly trends", "Show all columns", "Filter last 30 days"].map(hint => (
                    <button key={hint} className="hint-chip" onClick={() => setQuestion(hint)} disabled={!dbName}>{hint}</button>
                  ))}
                </div>
              </div>
                ...
                {isQuerying ? (
                  ...
                  <>
                    <div className="answer-header">
                      <div className="answer-label"><span className="answer-label-dot" />Result</div>
                    ...
                    <div className="answer-scroll">
                      {animateAnswer
                        ? <TypewriterMarkdown text={answer} />
                        : <div className="md-body">{renderMarkdown(answer)}</div>
                      }
                    </div>
                  </>
                ) : (
